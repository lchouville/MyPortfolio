...
export function normalizeSkillsData(skillsData) {
  ...
  const normalized = {
    hardSkills: {},
    softSkills: {},
    flat: { hardSkills: [], softSkills: [], interests: [] }, // ajout pour les expériences
  };
  ...
  // Interests
  if (skillsData.interests && skillsData.interests.categories) {
    normalized.interests = {};

    for (const [category, value] of Object.entries(skillsData.interests.categories)) {
      const items = value.items
        ? value.items
        : Array.isArray(value)
          ? value
      ...
      // Ajouter au flat (y compris les sous-détails)
      normalized.flat.interests.push(...items);
      items.forEach(item => {
        if (item.details) {
          normalized.flat.interests.push(...item.details);
        }
      });
    }
  }
